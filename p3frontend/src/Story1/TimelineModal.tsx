...
export class TimelineModal extends React.Component<TimelineModalProps> {
    ...
    render() {
        let currentWeek = dateDifferenceWeeks(new Date(this.props.batch.startDate),new Date(Date.now()))
        let remainingWeeks = dateDifferenceWeeks(new Date(Date.now()),new Date(this.props.batch.endDate))

        let activeAssociates = associatesGetActiveTotal(this.props.batch.associates,true)
        let inactiveAssociates = associatesGetActiveTotal(this.props.batch.associates,false)
